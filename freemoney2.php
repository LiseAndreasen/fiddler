...
function find_high_median() {
	global $buckets, $loops;
	$median_or_more = 0;
	while($median_or_more < $loops / 2) {
		$max_money = max(array_keys($buckets));
		$median_or_more += $buckets[$max_money];
		unset($buckets[$max_money]);
	}
	return $max_money;
}
